...
2. Pick an offer (Base mainnet `eip155:8453` for real use; `eip155:84532` for integration tests) and sign an EIP-3009 `TransferWithAuthorization` for exactly `amount` to `payTo`, `validBefore` within `maxTimeoutSeconds`, a fresh 32-byte nonce.
